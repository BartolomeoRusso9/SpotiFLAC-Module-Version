...
from tests.conftest import SPOTIFY_ALBUM, SPOTIFY_TRACK
...
class TestTrackMaxRetries:
    ...
    def test_retry_count_stored(self, retries, tmp_output_dir, mock_spotiflac) -> None:
    ...
    def test_zero_retries_means_no_extra_attempts(
        self,
        tmp_output_dir,
        mock_spotiflac,
    ) -> None:
    ...
    def test_high_retry_count_accepted(self, tmp_output_dir, mock_spotiflac) -> None:
        mock_cls, _ = mock_spotiflac
        mock_cls(
            url=SPOTIFY_TRACK,
            output_dir=tmp_output_dir,
...
class TestExponentialBackoff:
    def test_first_attempt_delay_is_2s(self) -> None:
        assert _backoff_delay(1) == 2

    def test_second_attempt_delay_is_4s(self) -> None:
        assert _backoff_delay(2) == 4

    def test_third_attempt_delay_is_8s(self) -> None:
        assert _backoff_delay(3) == 8

    def test_fourth_attempt_delay_is_16s(self) -> None:
        assert _backoff_delay(4) == 16

    def test_fifth_attempt_delay_is_30s_capped(self) -> None:
        assert _backoff_delay(5) == 30

    def test_delays_are_always_capped_at_30s(self) -> None:
    ...
    def test_total_delay_for_3_retries(self) -> None:
...
class TestTimeoutParameter:
    ...
    def test_timeout_accepted(self, timeout, tmp_output_dir, mock_spotiflac) -> None:
        mock_cls, _ = mock_spotiflac
        mock_cls(
            url=SPOTIFY_TRACK,
            output_dir=tmp_output_dir,
    ...
    def test_timeout_none_disables_cap(self, tmp_output_dir, mock_spotiflac) -> None:
        mock_cls, _ = mock_spotiflac
        mock_cls(
            url=SPOTIFY_TRACK,
            output_dir=tmp_output_dir,
    ...
    def test_timeout_is_per_track_not_per_session(self) -> None:
...
class TestLoopParameter:
    ...
    def test_loop_minutes_accepted(
        self,
        minutes,
        tmp_output_dir,
        mock_spotiflac,
    ...
    def test_loop_combined_with_retries(self, tmp_output_dir, mock_spotiflac) -> None:
    ...
    def test_loop_with_timeout_and_retries(
        self,
        tmp_output_dir,
        mock_spotiflac,
    ) -> None:
...
class TestProviderCyclingOnRetry:
    ...
    def test_three_providers_three_retries_yields_nine_attempts(self) -> None:
    ...
    def test_single_provider_retry_stays_on_that_provider(self) -> None:
        services = ["tidal"]
        retries = 3
