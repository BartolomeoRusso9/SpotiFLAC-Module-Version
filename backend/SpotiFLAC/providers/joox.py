...
# JOOX typically serves lossy streams; we request 999 and accept
# whatever the API returns, refusing only if the actual quality
...
class JooxProvider(BaseProvider):
    """
    Provider for GD Studio Music API (music-api.gdstudio.xyz) — JOOX source.

    JOOX is a popular streaming service in Southeast Asia and Hong Kong.
    Audio is typically served as MP3; lossless availability is limited.
    The provider requests br=999 and accepts the highest quality returned.
    ...
    def _search(self, query: str, count: int = 10) -> list[dict]:
        """Search for tracks on JOOX. Returns raw API result items."""
        try:
    ...
    def _get_stream(self, track_id: str) -> tuple[str, int]:
        """
        Request a stream URL from JOOX via GD Studio API.
        Returns (url, actual_br). url is empty string on failure.

        JOOX commonly returns 128/320 kbps MP3; lossless (740/999) is rare.
        We accept anything ≥ 128 kbps.
        """
        try:
            resp = self._session.get(
                _API_BASE,
            ...
            if actual_br < 64:
                logger.debug(
                    "[joox] Track %s returned br=%d — likely unavailable",
                    track_id, actual_br,
                )
                return "", actual_br
            return url, actual_br
        except Exception as exc:
            logger.debug("[joox] Stream fetch failed for id=%s: %s", track_id, exc)
        return "", 0
    ...
    def _get_lyric(self, lyric_id: str) -> str:
        """Fetch LRC lyrics (original language + optional Chinese translation)."""
        if not lyric_id:
            return ""
        try:
            resp = self._session.get(
            ...
            resp.raise_for_status()
            data = resp.json()
            # Prefer the original-language lyric; fall back to translation
    ...
    def _get_album_tracks(self, album_id: str) -> list[dict]:
        """
        Fetch the track list of a JOOX album using the joox_album source suffix.
        """
        try:
            resp = self._session.get(
                _API_BASE,
        ...
        try:
            ...
            if not dl_url:
                raise SpotiflacError(
                    ...
                    f"No stream available on JOOX for id={raw_track_id}",
                    self.name,
                )

            # ── 3. Determine file extension from quality ──────────────────
            # JOOX rarely serves lossless; default to .mp3
            ...
            # ── 8. Download ───────────────────────────────────────────────
            logger.info("[joox] Downloading '%s' (id=%s, br=%d)", metadata.title, raw_track_id, actual_br)
