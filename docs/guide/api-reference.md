...
| `save_canvas` | `bool` | `False` | Also save the track's Spotify Canvas (the short looping visual) as `<audio file's name>.mp4`, next to the track. Never embedded — see [Spotify Canvas](configuration.md#spotify-canvas). |
| `canvas_library_dir` | `str` | `None` | Also collect every canvas into this folder as `Artist - Title.mp4`. |
...
| `--save-canvas` | | off | Also save the track's Spotify Canvas as a video file next to it, under the audio file's own name. |
| `--canvas-dir` | `DIR` | — | Also collect every canvas into `DIR` as `Artist - Title.mp4`. |
