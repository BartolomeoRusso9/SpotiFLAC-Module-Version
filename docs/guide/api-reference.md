...
| `--log-level` | | `ERROR` | Console log level: `DEBUG`, `INFO`, `WARNING`, `ERROR`, `CRITICAL` (aliases like `WARN` and numeric values are accepted). Overrides `--verbose`, which is a shorthand for `DEBUG`. A level stored in a profile ranks below `--verbose`. |
