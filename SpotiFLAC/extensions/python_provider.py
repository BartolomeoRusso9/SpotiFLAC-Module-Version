...
def load_python_utilities(manager: ExtensionManager) -> None:
    ...
    for base_name in _UTILITY_ORDER:
        ext = next((c for c in installed if c.runtime == "python" and base_name.replace("-", "") in c.name.replace("-", "").lower()), None)
        if not ext:
        ...
        try:
            ...
            logger.warning(f"[Utilities] Impossibile caricare l'utility {base_name}: {e}")
...
class PythonExtensionProvider(BaseProvider):
    ...
    def __new__(cls, ext_id: str, *, ext_dir: str | None = None, **kwargs: Any):
        ...
        base_name = ext_id.replace("ext:", "").replace("-web", "").replace("-py", "").lower()
        ext = next((c for c in manager.list_installed() if c.runtime == "python" and base_name in c.name.lower()), None)
