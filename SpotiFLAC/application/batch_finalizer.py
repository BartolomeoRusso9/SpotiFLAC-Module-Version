...
class BatchFinalizer:
    ...
    async def finalize(self, output_dir: str, initial_m4a: set[Path] | None = None) -> None:
        removed = await asyncio.to_thread(self._remove_partial_files, output_dir, initial_m4a)
        if removed:
            logger.debug("[downloader] Removed %d leftover partial/invalid audio file(s)", removed)
    ...
    def _remove_partial_files(
        ...
        if not root.exists():
            return 0
        completed_paths = {Path(path).resolve() for path in self._completed_paths.values() if path}
        ...
        for path in part_candidates + m4a_candidates:
            if not path.is_file() or (path.suffix.lower() == ".m4a" and self._valid_m4a(path)):
                continue
            try:
                path.unlink()
                removed += 1
            except OSError as exc:
                logger.warning("[downloader] Could not remove partial file %s: %s", path, exc)
        return removed
        ...
        if action == "open_folder":
            ...
            try:
                ...
                if process.returncode:
                    logger.warning("[post-action] command exited with status %s", process.returncode)
