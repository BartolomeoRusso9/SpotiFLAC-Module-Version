...
# Re-exported under the wizard's old spellings: unused in this module now,
# but `tests/test_interactive_csv_picker.py` still reaches for them here.
# They go when the wizard does.
from .core.csv_picker import (
    CSV_BROWSE_WORDS as _CSV_BROWSE_WORDS,  # noqa: F401
)
from .core.csv_picker import (
    CSV_SCAN_LIMIT as _CSV_SCAN_LIMIT,  # noqa: F401
)
from .core.csv_picker import (
    CSV_SUFFIXES as _CSV_SUFFIXES,  # noqa: F401
)
from .core.csv_picker import (
    clean_path_input,
    csv_scan_dirs,
...
from .core.transcode import TRANSCODE_CHOICES, is_lossless, transcode_label
...
# The menu itself now lives in core/transcode.py, where the TUI and the GUI
# can read the same seven targets. These two names stay as aliases because
# the wizard and its tests have always spelled them this way.
_TRANSCODE_CHOICES = TRANSCODE_CHOICES
_transcode_label = transcode_label
...
def _installed_service_options() -> list[str]:
    ...
    is also what the GUI's Settings list and the TUI read — the menus have
    to offer the same providers, and the only way to be sure of that is for
    them to ask the same function.
    """
    return installed_service_ids()
...
# The path handling and the folder scan moved to core/csv_picker.py so the
# TUI could offer the same picker; these names stay as aliases because the
# wizard and its tests have always spelled them this way.
_clean_path_input = clean_path_input
_looks_like_csv_path = looks_like_csv_path
_short_dir = short_dir
_format_size = format_size
_csv_scan_dirs = csv_scan_dirs
_scan_csv_files = scan_csv_files
_read_csv_document = read_csv_document
...
# Shared with the CLI-preview builder and the TUI, which ask the same
# question of the same URL.
_is_playlist_url = is_playlist_url
...
def _print_cli_command(cfg: dict) -> None:
    ...
    The command itself is built by `core.cli_preview.format_command()`, which
    the TUI renders as a live panel rather than a parting line; all that is
    left here is putting it on the screen.
